...
# file uploader --- functionality to insert images to the DataBase


function file_upload_user($picture)
{
    ...
    if (in_array($fileExtension, $filesAllowed)) {
      if ($fileError === 0) {
        if ($fileSize < 500000) { //500kb this number is in bytes
          ...
          $fileNewName = uniqid('') . "." . $fileExtension; // 1233343434.jpg i.
          $destination = "../pictures/user/$fileNewName";
...
function file_upload_recipe($picture)
{
  $result = new stdClass(); //this object will carry status from file upload
  $result->fileName = 'meal.jpeg';
  $result->error = 1; //it could also be a boolean true/false
  //collect data from object $picture
  $fileName = $picture["name"];
  $fileType = $picture["type"];
    ...
    if (in_array($fileExtension, $filesAllowed)) {
      if ($fileError === 0) {
        if ($fileSize < 500000) { //500kb this number is in bytes
          ...
          $destination = "../pictures/recipies/$fileNewName";
          if (move_uploaded_file($fileTmpName, $destination)) {
            $result->error = 0;
            $result->fileName = $fileNewName;
            return $result;
